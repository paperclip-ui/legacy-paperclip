...
"use strict";
var __createBinding=this&&this.__createBinding||(Object.create?function(o,m,k,k2){if(k2===undefined)k2=k;Object.defineProperty(o,k2,{enumerable:true,get:function(){return m[k];}});}:function(o,m,k,k2){if(k2===undefined)k2=k;o[k2]=m[k];});var __exportStar=this&&this.__exportStar||function(m,exports){for(var p in m)if(p!=="default"&&!Object.prototype.hasOwnProperty.call(exports,p))__createBinding(exports,m,p);};Object.defineProperty(exports, "__esModule", ({value:true}));__exportStar(__webpack_require__(3827),exports);__exportStar(__webpack_require__(20827),exports);
...
"use strict";
Object.defineProperty(exports, "__esModule", ({value:true}));exports.PCMutationActionKind=void 0;var PCMutationActionKind;(function(PCMutationActionKind){PCMutationActionKind["ANNOTATIONS_CHANGED"]="ANNOTATIONS_CHANGED";PCMutationActionKind["EXPRESSION_DELETED"]="EXPRESSION_DELETED";PCMutationActionKind["CSS_DECLARATION_CHANGED"]="CSS_DECLARATION_CHANGED";})(PCMutationActionKind=exports.PCMutationActionKind||(exports.PCMutationActionKind={}));
...
"use strict";
Object.defineProperty(exports, "__esModule", ({value:true}));exports.editString=void 0;var editString=function(input,changes){var output=input;for(var i=0,n=changes.length;i<n;i++){var _a=changes[i],start=_a.start,end=_a.end,value=_a.value;var offsetStartIndex=start;var offsetEndIndex=end;var invalid=false;var insertion=start===end;// based on all of the previous edits, calculate where this edit is
for(var j=0;j<i;j++){var _b=changes[j],previousStartIndex=_b.start,previousEndIndex=_b.end,previousNewValue=_b.value;var prevInsertion=previousStartIndex===previousEndIndex;var startIndicesMatch=start===previousStartIndex;var endIndicesMatch=end===previousEndIndex;// input :  a b c d e f g h i
// prev  :     ^-------^
// ✔     :     ^
var insertBeginning=startIndicesMatch&&insertion;// input :  a b c d e f g h i
// prev  :     ^-------^
// ✔     :             ^
var insertEnd=endIndicesMatch&&insertion;// input :  a b c d e f g h i
// prev  :     ^
// ✔     :     ^-------^
var prevInsertBeginning=startIndicesMatch&&prevInsertion;// input :  a b c d e f g h i
// prev  :     ^
// ✔     :     ^-------^
var prevInsertEnd=endIndicesMatch&&prevInsertion;var currOrPrevInserting=insertBeginning||insertEnd||prevInsertBeginning||prevInsertEnd;// input :  a b c d e f g h i
// prev  :         ^-------^
// ✔     :     ^-------^
if(previousStartIndex<end&&previousStartIndex>start){offsetEndIndex=offsetEndIndex-(end-previousStartIndex);}// input :  a b c d e f g h i
// prev  :   ^-----^
// ✔     :       ^-------^
if(previousEndIndex>start&&previousEndIndex<end){offsetStartIndex=offsetStartIndex+(previousEndIndex-start);}// Invalid edit because previous replacement
...
// ✘     :             ^
// ✘     :   ^-----------^
if((start>=previousStartIndex&&end<=previousEndIndex||start<previousStartIndex&&end>=previousEndIndex)&&!currOrPrevInserting){invalid=true;break;}// input :  a b c d e f g h
// prev  :     ^-----^
// ✔     :       ^-----^
// ✔     :           ^---^
// ✔     :               ^-^
...
// prev  : ^---^
// ✔     :   ^---^
if(previousStartIndex<=start&&end>previousStartIndex){var prevValueLengthDelta=previousNewValue.length-(previousEndIndex-previousStartIndex);// shift left or right
offsetStartIndex=Math.max(0,offsetStartIndex+prevValueLengthDelta);offsetEndIndex=Math.max(0,offsetEndIndex+prevValueLengthDelta);}}if(!invalid){output=output.substr(0,offsetStartIndex)+value+output.substr(offsetEndIndex);}}return output;};exports.editString=editString;
...
"use strict";
Object.defineProperty(exports, "__esModule", ({value:true}));exports.PCSourceWriter=void 0;var mutations_1=__webpack_require__(20827);var paperclip_utils_1=__webpack_require__(85402);var string_editor_1=__webpack_require__(41376);var ANNOTATION_KEYS=["title","width","height","x","y"];// TODO: add more tests here
var PCSourceWriter=/** @class */function(){function PCSourceWriter(_engine){this._engine=_engine;}PCSourceWriter.prototype.apply=function(mutations){var changes=this._getContentChanges(mutations);for(var uri in changes){var newContent=(0,string_editor_1.editString)(this._engine.getVirtualContent(uri),changes[uri]);this._engine.updateVirtualFileContent(uri,newContent);}// return changes so that
return changes;};PCSourceWriter.prototype._getContentChanges=function(mutations){var _a;var changes=[];var engine=this._engine;for(var _i=0,mutations_2=mutations;_i<mutations_2.length;_i++){var _b=mutations_2[_i],targetId=_b.targetId,action=_b.action;var _c=engine.getExpressionById(targetId),uri=_c[0],targetAst=_c[1];var documentAst=engine.getLoadedAst(uri);var textSource={uri:uri,range:targetAst.range};switch(action.kind){case mutations_1.PCMutationActionKind.ANNOTATIONS_CHANGED:{changes.push(this._getAnnotationChange(textSource,(_a=(0,paperclip_utils_1.getPCNodeAnnotations)((0,paperclip_utils_1.getNodeById)(targetAst.id,documentAst),documentAst))===null||_a===void 0?void 0:_a.range,action.annotations));break;}case mutations_1.PCMutationActionKind.CSS_DECLARATION_CHANGED:{changes.push(this._getCSSDeclarationChange(textSource,action));break;}case mutations_1.PCMutationActionKind.EXPRESSION_DELETED:{changes.push.apply(changes,this._getExpressionDeletedChanged(textSource,targetAst));break;}}}var changesByUri={};for(var _d=0,changes_1=changes;_d<changes_1.length;_d++){var change=changes_1[_d];if(!changesByUri[change.uri]){changesByUri[change.uri]=[];}changesByUri[change.uri].push(change);}return changesByUri;};PCSourceWriter.prototype._getExpressionDeletedChanged=function(exprTextSource,ast){var node=getAssocNode(exprTextSource,ast);var parent=(0,paperclip_utils_1.getParentNode)(node,ast);var childIndex=parent.children.findIndex(function(child){return child===node;});var changes=[];var beforeChild=childIndex>0?parent.children[childIndex-1]:null;// if before child is a comment, then assume it's an annotation
if(beforeChild&&beforeChild.nodeKind===paperclip_utils_1.NodeKind.Comment){changes.push({uri:exprTextSource.uri,start:beforeChild.range.start.pos,end:beforeChild.range.end.pos,value:""});}changes.push({uri:exprTextSource.uri,start:exprTextSource.range.start.pos,end:exprTextSource.range.end.pos,value:""});return changes;};PCSourceWriter.prototype._getCSSDeclarationChange=function(exprTextSource,action){return{uri:exprTextSource.uri,start:exprTextSource.range.start.pos,end:exprTextSource.range.end.pos,value:"".concat(action.name,": ").concat(action.value,";")};};PCSourceWriter.prototype._getAnnotationChange=function(exprTextSource,annotationsRange,annotations){var buffer=["<!--\n"];for(var key in annotations){var chunk=["  @".concat(key," ")];var value=annotations[key];if(Array.isArray(value)||typeof value==="string"||typeof value==="number"){chunk.push(JSON.stringify(value));}else{chunk.push("{ ");var items=[];var sortedKeys=Object.keys(value).sort(function(a,b){return ANNOTATION_KEYS.indexOf(a)<ANNOTATION_KEYS.indexOf(b)?-1:1;});for(var _i=0,sortedKeys_1=sortedKeys;_i<sortedKeys_1.length;_i++){var k=sortedKeys_1[_i];items.push("".concat(k,": ").concat(JSON.stringify(value[k])));}chunk.push(items.join(", ")," }");}buffer.push(chunk.join(""),"\n");}buffer.push("-->");// insertion - give it some padding
if(!annotationsRange){buffer.unshift("\n");buffer.push("\n");}return{uri:exprTextSource.uri,start:annotationsRange?annotationsRange.start.pos:exprTextSource.range.start.pos,end:annotationsRange?annotationsRange.end.pos:exprTextSource.range.start.pos,// newline may have been clipped off, so re-add if that happens
value:buffer.join("")};};return PCSourceWriter;}();exports.PCSourceWriter=PCSourceWriter;var getAssocNode=function(exprTextSource,root){var foundExpr;(0,paperclip_utils_1.traverseExpression)(root,function(node){if(node.range.start.pos===exprTextSource.range.start.pos&&node.range.end.pos===exprTextSource.range.end.pos){foundExpr=node;return false;}});// should NOT happen
if(!foundExpr){console.error("[PCSourceWriter] Cannot find associated node, content is likely out of sync with visual editor.");}return foundExpr;};
...
"use strict";
Object.defineProperty(exports, "__esModule", ({value:true}));exports.getOutputFile=exports.getPaperclipConfigIncludes=void 0;var path=__webpack_require__(71017);var utils_1=__webpack_require__(95323);var getPaperclipConfigIncludes=function(config,cwd){if(config.srcDir){return[(0,utils_1.paperclipSourceGlobPattern)(path.join(cwd,config.srcDir))];}// if (config.include) {
//   return config.include.map(inc => path.join(cwd, inc));
// }
return[path.join((0,utils_1.paperclipSourceGlobPattern)(cwd))];};exports.getPaperclipConfigIncludes=getPaperclipConfigIncludes;var getOutputFile=function(filePath,config,cwd){var _a;var outFilePath=((_a=config.compilerOptions)===null||_a===void 0?void 0:_a.outDir)?filePath.replace(path.join(cwd,config.srcDir),path.join(cwd,config.compilerOptions.outDir)):filePath;return outFilePath;};exports.getOutputFile=getOutputFile;
...
return[readJSONSync(fs)(uri),configUrl];};};exports.resolvePCConfig=resolvePCConfig;var resolveModule=function(fs){return function(fromPath,moduleRelativePath,resolveOutput){var _a;// need to parse each time in case config changed.
var _b=(0,exports.resolvePCConfig)(fs)(fromPath)||[],config=_b[0],configUrl=_b[1];if(!config){return null;}var configPathDir=path.dirname((0,utils_1.stripFileProtocol)(configUrl));var moduleFileUrl=url.pathToFileURL(path.normalize(path.join(configPathDir,config.srcDir,moduleRelativePath)));// FIRST look for modules in the sourceDirectory
...
__webpack_unused_export__ = ({value:true});exports.Hz=exports.XN=void 0;var fileURLToPath=function(url){if(typeof url!=="string"){url=url.href;}return url.substr("file://".length);};exports.XN=fileURLToPath;var pathToFileURL=function(filePath){if(filePath.charAt(0)!=="/"){filePath="/"+filePath;}return{href:"file://".concat(filePath)};};exports.Hz=pathToFileURL;
...
"use strict";
Object.defineProperty(exports, "__esModule", ({value:true}));exports.traverseJSExpression=exports.JsConjunctionOperatorKind=exports.JsExpressionKind=void 0;var ast_1=__webpack_require__(72070);var JsExpressionKind;(function(JsExpressionKind){JsExpressionKind["Node"]="Node";JsExpressionKind["Reference"]="Reference";JsExpressionKind["Array"]="Array";JsExpressionKind["Object"]="Object";JsExpressionKind["String"]="String";JsExpressionKind["Number"]="Number";JsExpressionKind["Boolean"]="Boolean";JsExpressionKind["Conjunction"]="Conjunction";JsExpressionKind["Not"]="Not";JsExpressionKind["Group"]="Group";})(JsExpressionKind=exports.JsExpressionKind||(exports.JsExpressionKind={}));var JsConjunctionOperatorKind;(function(JsConjunctionOperatorKind){JsConjunctionOperatorKind["And"]="And";JsConjunctionOperatorKind["Or"]="Or";})(JsConjunctionOperatorKind=exports.JsConjunctionOperatorKind||(exports.JsConjunctionOperatorKind={}));var traverseJSExpression=function(expr,each){if(expr.jsKind===JsExpressionKind.Conjunction){return(0,exports.traverseJSExpression)(expr.left,each)&&(0,exports.traverseJSExpression)(expr.right,each);}else if(expr.jsKind===JsExpressionKind.Array){for(var _i=0,_a=expr.values;_i<_a.length;_i++){var value=_a[_i];if((0,exports.traverseJSExpression)(value,each)===false){return false;}}}else if(expr.jsKind===JsExpressionKind.Node){return(0,ast_1.traverseExpression)(expr,each);}return true;};exports.traverseJSExpression=traverseJSExpression;
...
"use strict";
Object.defineProperty(exports, "__esModule", ({value:true}));exports.computeVirtJSValue=exports.toVirtJsValue=exports.computeVirtJSObject=exports.VirtJsObjectKind=void 0;var memo_1=__webpack_require__(94841);var VirtJsObjectKind;(function(VirtJsObjectKind){VirtJsObjectKind["JsObject"]="JsObject";VirtJsObjectKind["JsArray"]="JsArray";VirtJsObjectKind["JsBoolean"]="JsBoolean";VirtJsObjectKind["JsNumber"]="JsNumber";VirtJsObjectKind["JsString"]="JsString";})(VirtJsObjectKind=exports.VirtJsObjectKind||(exports.VirtJsObjectKind={}));exports.computeVirtJSObject=(0,memo_1.memoize)(function(obj){var values={};for(var key in obj.values){values[key]=(0,exports.computeVirtJSValue)(obj.values[key]);}return values;});exports.toVirtJsValue=(0,memo_1.memoize)(function(value){if(Array.isArray(value)){return{kind:VirtJsObjectKind.JsArray,values:value.map(exports.toVirtJsValue)};}else if(value&&typeof value==="object"){var values={};for(var k in value){values[k]=(0,exports.toVirtJsValue)(value[k]);}return{kind:VirtJsObjectKind.JsObject,values:values};}else if(typeof value==="number"){return{kind:VirtJsObjectKind.JsNumber,value:value};}else if(typeof value==="string"){return{kind:VirtJsObjectKind.JsString,value:value};}else if(typeof value==="boolean"){return{kind:VirtJsObjectKind.JsBoolean,value:value};}});exports.computeVirtJSValue=(0,memo_1.memoize)(function(obj){switch(obj.kind){case VirtJsObjectKind.JsObject:{return(0,exports.computeVirtJSObject)(obj);}case VirtJsObjectKind.JsArray:{return obj.values.map(exports.computeVirtJSValue);}case VirtJsObjectKind.JsString:case VirtJsObjectKind.JsBoolean:case VirtJsObjectKind.JsNumber:{return obj.value;}}});
...
/* harmony export */   "__wbg_call_346669c262382ad7": () => (/* binding */ __wbg_call_346669c262382ad7),
...
/* harmony export */ });
/* harmony import */ var _paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(21877);
/* module decorator */ module = __webpack_require__.hmd(module);
/* provided dependency */ var TextDecoder = __webpack_require__(73837)["TextDecoder"];
/* provided dependency */ var TextEncoder = __webpack_require__(73837)["TextEncoder"];
var __webpack_async_dependencies__ = __webpack_handle_async_dependencies__([_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__]);
_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__ = (__webpack_async_dependencies__.then ? await __webpack_async_dependencies__ : __webpack_async_dependencies__)[0];
const heap=new Array(32).fill(undefined);heap.push(undefined,null,true,false);function getObject(idx){return heap[idx];}let heap_next=heap.length;function dropObject(idx){if(idx<36)return;heap[idx]=heap_next;heap_next=idx;}function takeObject(idx){const ret=getObject(idx);dropObject(idx);return ret;}function addHeapObject(obj){if(heap_next===heap.length)heap.push(heap.length+1);const idx=heap_next;heap_next=heap[idx];heap[idx]=obj;return idx;}const lTextDecoder=typeof TextDecoder==='undefined'?(0,module.require)('util').TextDecoder:TextDecoder;let cachedTextDecoder=new lTextDecoder('utf-8',{ignoreBOM:true,fatal:true});cachedTextDecoder.decode();let cachegetUint8Memory0=null;function getUint8Memory0(){if(cachegetUint8Memory0===null||cachegetUint8Memory0.buffer!==_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.memory.buffer){cachegetUint8Memory0=new Uint8Array(_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.memory.buffer);}return cachegetUint8Memory0;}function getStringFromWasm0(ptr,len){return cachedTextDecoder.decode(getUint8Memory0().subarray(ptr,ptr+len));}let WASM_VECTOR_LEN=0;const lTextEncoder=typeof TextEncoder==='undefined'?(0,module.require)('util').TextEncoder:TextEncoder;let cachedTextEncoder=new lTextEncoder('utf-8');const encodeString=typeof cachedTextEncoder.encodeInto==='function'?function(arg,view){return cachedTextEncoder.encodeInto(arg,view);}:function(arg,view){const buf=cachedTextEncoder.encode(arg);view.set(buf);return{read:arg.length,written:buf.length};};function passStringToWasm0(arg,malloc,realloc){if(realloc===undefined){const buf=cachedTextEncoder.encode(arg);const ptr=malloc(buf.length);getUint8Memory0().subarray(ptr,ptr+buf.length).set(buf);WASM_VECTOR_LEN=buf.length;return ptr;}let len=arg.length;let ptr=malloc(len);const mem=getUint8Memory0();let offset=0;for(;offset<len;offset++){const code=arg.charCodeAt(offset);if(code>0x7F)break;mem[ptr+offset]=code;}if(offset!==len){if(offset!==0){arg=arg.slice(offset);}ptr=realloc(ptr,len,len=offset+arg.length*3);const view=getUint8Memory0().subarray(ptr+offset,ptr+len);const ret=encodeString(arg,view);offset+=ret.written;}WASM_VECTOR_LEN=offset;return ptr;}let cachegetInt32Memory0=null;function getInt32Memory0(){if(cachegetInt32Memory0===null||cachegetInt32Memory0.buffer!==_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.memory.buffer){cachegetInt32Memory0=new Int32Array(_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.memory.buffer);}return cachegetInt32Memory0;}function isLikeNone(x){return x===undefined||x===null;}function debugString(val){// primitive types
const type=typeof val;if(type=='number'||type=='boolean'||val==null){return`${val}`;}if(type=='string'){return`"${val}"`;}if(type=='symbol'){const description=val.description;if(description==null){return'Symbol';}else{return`Symbol(${description})`;}}if(type=='function'){const name=val.name;if(typeof name=='string'&&name.length>0){return`Function(${name})`;}else{return'Function';}}// objects
...
if(val instanceof Error){return`${val.name}: ${val.message}\n${val.stack}`;}// TODO we could test for more things here, like `Set`s and `Map`s.
return className;}function handleError(f,args){try{return f.apply(this,args);}catch(e){_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_exn_store(addHeapObject(e));}}function getArrayU8FromWasm0(ptr,len){return getUint8Memory0().subarray(ptr/1,ptr/1+len);}let cachegetUint32Memory0=null;function getUint32Memory0(){if(cachegetUint32Memory0===null||cachegetUint32Memory0.buffer!==_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.memory.buffer){cachegetUint32Memory0=new Uint32Array(_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.memory.buffer);}return cachegetUint32Memory0;}function passArray32ToWasm0(arg,malloc){const ptr=malloc(arg.length*4);getUint32Memory0().set(arg,ptr/4);WASM_VECTOR_LEN=arg.length;return ptr;}/**
*/const NativeEngineMode=Object.freeze({SingleFrame:0,"0":"SingleFrame",MultiFrame:1,"1":"MultiFrame"});/**
*/class NativeEngine{static __wrap(ptr){const obj=Object.create(NativeEngine.prototype);obj.ptr=ptr;return obj;}__destroy_into_raw(){const ptr=this.ptr;this.ptr=0;return ptr;}free(){const ptr=this.__destroy_into_raw();_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbg_nativeengine_free(ptr);}/**
    ...
    */static new(read_file,file_exists,resolve_file,get_lint_config,engine_mode){var ret=_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.nativeengine_new(addHeapObject(read_file),addHeapObject(file_exists),addHeapObject(resolve_file),addHeapObject(get_lint_config),engine_mode);return NativeEngine.__wrap(ret);}/**
    * @param {string} uri
    * @returns {any}
    */load(uri){var ptr0=passStringToWasm0(uri,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_malloc,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_realloc);var len0=WASM_VECTOR_LEN;var ret=_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.nativeengine_load(this.ptr,ptr0,len0);return takeObject(ret);}/**
    * @param {string} uri
    * @returns {any}
    */run(uri){var ptr0=passStringToWasm0(uri,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_malloc,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_realloc);var len0=WASM_VECTOR_LEN;var ret=_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.nativeengine_run(this.ptr,ptr0,len0);return takeObject(ret);}/**
    * @returns {any}
    */get_graph_uris(){var ret=_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.nativeengine_get_graph_uris(this.ptr);return takeObject(ret);}/**
    */reset(){_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.nativeengine_reset(this.ptr);}/**
    * @param {Function} listener
    */add_listener(listener){_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.nativeengine_add_listener(this.ptr,addHeapObject(listener));}/**
    * @param {string} uri
    * @returns {any}
    */lint_file(uri){var ptr0=passStringToWasm0(uri,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_malloc,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_realloc);var len0=WASM_VECTOR_LEN;var ret=_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.nativeengine_lint_file(this.ptr,ptr0,len0);return takeObject(ret);}/**
    * @param {Uint32Array} path
    * @param {string} uri
    * @returns {any}
    */get_virtual_node_source_info(path,uri){var ptr0=passArray32ToWasm0(path,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_malloc);var len0=WASM_VECTOR_LEN;var ptr1=passStringToWasm0(uri,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_malloc,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_realloc);var len1=WASM_VECTOR_LEN;var ret=_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.nativeengine_get_virtual_node_source_info(this.ptr,ptr0,len0,ptr1,len1);return takeObject(ret);}/**
    * @param {string} uri
    * @returns {any}
    */get_loaded_ast(uri){var ptr0=passStringToWasm0(uri,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_malloc,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_realloc);var len0=WASM_VECTOR_LEN;var ret=_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.nativeengine_get_loaded_ast(this.ptr,ptr0,len0);return takeObject(ret);}/**
    * @param {string} uri
    * @returns {any}
    */get_dependency(uri){var ptr0=passStringToWasm0(uri,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_malloc,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_realloc);var len0=WASM_VECTOR_LEN;var ret=_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.nativeengine_get_dependency(this.ptr,ptr0,len0);return takeObject(ret);}/**
    * @param {string} content
    * @param {string} uri
    * @returns {any}
    */parse_content(content,uri){var ptr0=passStringToWasm0(content,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_malloc,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_realloc);var len0=WASM_VECTOR_LEN;var ptr1=passStringToWasm0(uri,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_malloc,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_realloc);var len1=WASM_VECTOR_LEN;var ret=_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.nativeengine_parse_content(this.ptr,ptr0,len0,ptr1,len1);return takeObject(ret);}/**
    * @param {string} uri
    * @returns {any}
    */parse_file(uri){var ptr0=passStringToWasm0(uri,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_malloc,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_realloc);var len0=WASM_VECTOR_LEN;var ret=_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.nativeengine_parse_file(this.ptr,ptr0,len0);return takeObject(ret);}/**
    */purge_unlinked_files(){_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.nativeengine_purge_unlinked_files(this.ptr);}/**
    * @param {string} id
    * @returns {any}
    */get_expression_by_id(id){var ptr0=passStringToWasm0(id,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_malloc,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_realloc);var len0=WASM_VECTOR_LEN;var ret=_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.nativeengine_get_expression_by_id(this.ptr,ptr0,len0);return takeObject(ret);}/**
    * @param {Uint32Array} path
    * @param {string} uri
    * @param {number} screen_width
    * @returns {any}
    */inspect_node_styles(path,uri,screen_width){var ptr0=passArray32ToWasm0(path,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_malloc);var len0=WASM_VECTOR_LEN;var ptr1=passStringToWasm0(uri,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_malloc,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_realloc);var len1=WASM_VECTOR_LEN;var ret=_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.nativeengine_inspect_node_styles(this.ptr,ptr0,len0,ptr1,len1,screen_width);return takeObject(ret);}/**
    * @param {string} uri
    * @param {string} content
    */update_virtual_file_content(uri,content){var ptr0=passStringToWasm0(uri,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_malloc,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_realloc);var len0=WASM_VECTOR_LEN;var ptr1=passStringToWasm0(content,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_malloc,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_realloc);var len1=WASM_VECTOR_LEN;_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.nativeengine_update_virtual_file_content(this.ptr,ptr0,len0,ptr1,len1);}}function __wbg_new_693216e109162396(){var ret=new Error();return addHeapObject(ret);};function __wbg_stack_0ddaca5d1abfb52f(arg0,arg1){var ret=getObject(arg1).stack;var ptr0=passStringToWasm0(ret,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_malloc,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_realloc);var len0=WASM_VECTOR_LEN;getInt32Memory0()[arg0/4+1]=len0;getInt32Memory0()[arg0/4+0]=ptr0;};function __wbg_error_09919627ac0992f5(arg0,arg1){try{console.error(getStringFromWasm0(arg0,arg1));}finally{_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_free(arg0,arg1);}};function __wbindgen_object_drop_ref(arg0){takeObject(arg0);};function __wbindgen_object_clone_ref(arg0){var ret=getObject(arg0);return addHeapObject(ret);};function __wbg_process_2f24d6544ea7b200(arg0){var ret=getObject(arg0).process;return addHeapObject(ret);};function __wbindgen_is_object(arg0){const val=getObject(arg0);var ret=typeof val==='object'&&val!==null;return ret;};function __wbg_versions_6164651e75405d4a(arg0){var ret=getObject(arg0).versions;return addHeapObject(ret);};function __wbg_node_4b517d861cbcb3bc(arg0){var ret=getObject(arg0).node;return addHeapObject(ret);};function __wbindgen_is_string(arg0){var ret=typeof getObject(arg0)==='string';return ret;};function __wbg_modulerequire_3440a4bcf44437db(){return handleError(function(arg0,arg1){var ret=__webpack_require__(33705)(getStringFromWasm0(arg0,arg1));return addHeapObject(ret);},arguments);};function __wbg_crypto_98fc271021c7d2ad(arg0){var ret=getObject(arg0).crypto;return addHeapObject(ret);};function __wbg_msCrypto_a2cdb043d2bfe57f(arg0){var ret=getObject(arg0).msCrypto;return addHeapObject(ret);};function __wbg_newwithlength_929232475839a482(arg0){var ret=new Uint8Array(arg0>>>0);return addHeapObject(ret);};function __wbg_self_c6fbdfc2918d5e58(){return handleError(function(){var ret=self.self;return addHeapObject(ret);},arguments);};function __wbg_window_baec038b5ab35c54(){return handleError(function(){var ret=window.window;return addHeapObject(ret);},arguments);};function __wbg_globalThis_3f735a5746d41fbd(){return handleError(function(){var ret=globalThis.globalThis;return addHeapObject(ret);},arguments);};function __wbg_global_1bc0b39582740e95(){return handleError(function(){var ret=global.global;return addHeapObject(ret);},arguments);};function __wbindgen_is_undefined(arg0){var ret=getObject(arg0)===undefined;return ret;};function __wbg_newnoargs_be86524d73f67598(arg0,arg1){var ret=new Function(getStringFromWasm0(arg0,arg1));return addHeapObject(ret);};function __wbg_call_888d259a5fefc347(){return handleError(function(arg0,arg1){var ret=getObject(arg0).call(getObject(arg1));return addHeapObject(ret);},arguments);};function __wbindgen_json_parse(arg0,arg1){var ret=JSON.parse(getStringFromWasm0(arg0,arg1));return addHeapObject(ret);};function __wbindgen_string_new(arg0,arg1){var ret=getStringFromWasm0(arg0,arg1);return addHeapObject(ret);};function __wbg_call_346669c262382ad7(){return handleError(function(arg0,arg1,arg2){var ret=getObject(arg0).call(getObject(arg1),getObject(arg2));return addHeapObject(ret);},arguments);};function __wbindgen_json_serialize(arg0,arg1){const obj=getObject(arg1);var ret=JSON.stringify(obj===undefined?null:obj);var ptr0=passStringToWasm0(ret,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_malloc,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_realloc);var len0=WASM_VECTOR_LEN;getInt32Memory0()[arg0/4+1]=len0;getInt32Memory0()[arg0/4+0]=ptr0;};function __wbg_call_8a893cac80deeb51(){return handleError(function(arg0,arg1,arg2,arg3){var ret=getObject(arg0).call(getObject(arg1),getObject(arg2),getObject(arg3));return addHeapObject(ret);},arguments);};function __wbindgen_string_get(arg0,arg1){const obj=getObject(arg1);var ret=typeof obj==='string'?obj:undefined;var ptr0=isLikeNone(ret)?0:passStringToWasm0(ret,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_malloc,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_realloc);var len0=WASM_VECTOR_LEN;getInt32Memory0()[arg0/4+1]=len0;getInt32Memory0()[arg0/4+0]=ptr0;};function __wbindgen_boolean_get(arg0){const v=getObject(arg0);var ret=typeof v==='boolean'?v?1:0:2;return ret;};function __wbg_randomFillSync_64cc7d048f228ca8(){return handleError(function(arg0,arg1,arg2){getObject(arg0).randomFillSync(getArrayU8FromWasm0(arg1,arg2));},arguments);};function __wbg_subarray_8b658422a224f479(arg0,arg1,arg2){var ret=getObject(arg0).subarray(arg1>>>0,arg2>>>0);return addHeapObject(ret);};function __wbg_getRandomValues_98117e9a7e993920(){return handleError(function(arg0,arg1){getObject(arg0).getRandomValues(getObject(arg1));},arguments);};function __wbg_length_1eb8fc608a0d4cdb(arg0){var ret=getObject(arg0).length;return ret;};function __wbindgen_memory(){var ret=_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.memory;return addHeapObject(ret);};function __wbg_buffer_397eaa4d72ee94dd(arg0){var ret=getObject(arg0).buffer;return addHeapObject(ret);};function __wbg_new_a7ce447f15ff496f(arg0){var ret=new Uint8Array(getObject(arg0));return addHeapObject(ret);};function __wbg_set_969ad0a60e51d320(arg0,arg1,arg2){getObject(arg0).set(getObject(arg1),arg2>>>0);};function __wbindgen_debug_string(arg0,arg1){var ret=debugString(getObject(arg1));var ptr0=passStringToWasm0(ret,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_malloc,_paperclip_bg_wasm__WEBPACK_IMPORTED_MODULE_0__.__wbindgen_realloc);var len0=WASM_VECTOR_LEN;getInt32Memory0()[arg0/4+1]=len0;getInt32Memory0()[arg0/4+0]=ptr0;};function __wbindgen_throw(arg0,arg1){throw new Error(getStringFromWasm0(arg0,arg1));};
});
...
var sockAdapter=function(worker){return{onMessage:function(listener){// is on the server
var onMessage=function(message){listener(JSON.parse(message));};// is on the client
if(!worker.on){return spy(worker,"onmessage",function(event){onMessage(event.data);});}worker.on("data",onMessage);return function(){return worker.off("data",onMessage);};},send:function(message){(worker.send||worker.write).call(worker,JSON.stringify(message));}};};exports.sockAdapter=sockAdapter;var remoteChannel=function(name){var requestName="".concat(name,":request");var responseName="".concat(name,":response");return function(chan){var call=function(payload){var id=Math.random();return new Promise(function(resolve,reject){var onMessage=function(message){if(message.id===id){disposeListener();if(message.error){reject(message.error);}else{resolve(message.payload);}}};var disposeListener=chan.onMessage(onMessage);chan.send({name:requestName,payload:payload,id:id});});};var listen=function(call){var dispose=chan.onMessage(function(message){return __awaiter(void 0,void 0,void 0,function(){var _a,_b,error_1;var _c;return __generator(this,function(_d){switch(_d.label){case 0:if(!(message.name===requestName))return[3/*break*/,4];_d.label=1;case 1:_d.trys.push([1,3,,4]);_b=(_a=chan).send;_c={name:responseName,id:message.id};return[4/*yield*/,call(message.payload)];case 2:_b.apply(_a,[(_c.payload=_d.sent(),_c)]);return[3/*break*/,4];case 3:error_1=_d.sent();chan.send({name:responseName,id:message.id,error:error_1});return[3/*break*/,4];case 4:return[2/*return*/];}});});});return{dispose:dispose};};return{call:call,listen:listen};};};exports.remoteChannel=remoteChannel;
...
var __webpack_instantiate__ = ([WEBPACK_IMPORTED_MODULE_0]) => {
	return __webpack_require__.v(exports, module.id, "1365b19500fd0a0c", {
		"./paperclip_bg.js": {
			...
			"__wbg_call_346669c262382ad7": WEBPACK_IMPORTED_MODULE_0.__wbg_call_346669c262382ad7,
...
__webpack_require__.a(module, (__webpack_handle_async_dependencies__) => {
	/* harmony import */ var WEBPACK_IMPORTED_MODULE_0 = __webpack_require__(10700);
...
;// CONCATENATED MODULE: ../paperclip-repl/src/controllers/worker-connection.ts
class WindowConnection{_em=new external_events_.EventEmitter();constructor(_worker){this._worker=_worker;_worker.onmessage=message=>{this._em.emit("message",message.data);};}async send(message){this._worker.postMessage(message);}onMessage(listener){this._em.on("message",listener);return()=>this._em.off("message",listener);}onOpen(listener){listener();return()=>null;}dispose(){}}
// EXTERNAL MODULE: ../tandem-common/lib/rpc.js
var rpc = __webpack_require__(79558);
;// CONCATENATED MODULE: ../tandem-designer/src/rpc/channels.ts
const inspectNodeStyleChannel=(0,rpc.remoteChannel)("inspectNodeStyleChannel");const setProjectIdChannel=(0,rpc.remoteChannel)("setProjectIdChannel");const revealNodeSourceChannel=(0,rpc.remoteChannel)("revealNodeSourceChannel");const revealNodeSourceByIdChannel=(0,rpc.remoteChannel)("revealNodeSourceByIdChannel");const editPCSourceChannel=(0,rpc.remoteChannel)("editPCSourceChannel");const editCodeChannel=(0,rpc.remoteChannel)("editCodeChannel");const popoutWindowChannel=(0,rpc.remoteChannel)("popoutWindowChannel");const eventsChannel=(0,rpc.remoteChannel)("eventsChannel");const commitChangesChannel=(0,rpc.remoteChannel)("commitChangesChannel");const setBranchChannel=(0,rpc.remoteChannel)("setBranchChannel");const getAllScreensChannel=(0,rpc.remoteChannel)("getAllScreensChannel");const helloChannel=(0,rpc.remoteChannel)("helloChannel");const loadDirectoryChannel=(0,rpc.remoteChannel)("loadDirectoryChannel");const openFileChannel=(0,rpc.remoteChannel)("openFileChannel");const loadVirtualNodeSourcesChannel=(0,rpc.remoteChannel)("loadVirtualNodeSourcesChannel");
;// CONCATENATED MODULE: ../tandem-designer/src/sagas/rpc/channels.ts
class Channels{constructor(connection){this.hello=helloChannel(connection);this.inspectNodeStyle=inspectNodeStyleChannel(connection);this.revealNodeSource=revealNodeSourceChannel(connection);this.popoutWindow=popoutWindowChannel(connection);this.getAllScreens=getAllScreensChannel(connection);this.loadDirectory=loadDirectoryChannel(connection);this.commitChanges=commitChangesChannel(connection);this.openFile=openFileChannel(connection);this.loadVirtualNodeSources=loadVirtualNodeSourcesChannel(connection);this.revealNodeSourceById=revealNodeSourceByIdChannel(connection);this.editPCSource=editPCSourceChannel(connection);this.events=eventsChannel(connection);this.editCode=editCodeChannel(connection);}}
...
function _interopRequireWildcard(obj, nodeInterop) {
  ...
  if (obj === null || helpers_typeof(obj) !== "object" && typeof obj !== "function") {
    return {
      "default": obj
    };
  }
...
;// CONCATENATED MODULE: ../paperclip/esm/core/infer.js
// TODO - this should be built in rust
var InferenceKind;(function(InferenceKind){InferenceKind[InferenceKind["Shape"]=0]="Shape";InferenceKind[InferenceKind["Array"]=1]="Array";InferenceKind[InferenceKind["Any"]=2]="Any";})(InferenceKind||(InferenceKind={}));const createShapeInference=(properties={},fromSpread=false)=>({kind:InferenceKind.Shape,fromSpread,properties});const createAnyInference=()=>({kind:InferenceKind.Any});const ANY_INFERENCE=createAnyInference();const SPREADED_SHAPE_INFERENCE=createShapeInference({},true);const addShapeInferenceProperty=(part,value,shape)=>{var _a,_b;return Object.assign(Object.assign({},shape),{properties:Object.assign(Object.assign({},shape.properties),{[part.name]:{value,optional:((_a=shape.properties[part.name])===null||_a===void 0?void 0:_a.optional)===false?(_b=shape.properties[part.name])===null||_b===void 0?void 0:_b.optional:part.optional}})});};const mergeShapeInference=(existing,extended)=>{if(extended.kind===InferenceKind.Any){return existing;}if(extended.kind===InferenceKind.Array){console.error(`Conflict: can't access properties of arra`);// ERRROR!
return existing;}return Object.assign(Object.assign({},existing),{properties:Object.assign(Object.assign({},existing.properties),extended.properties)});};const addInferenceProperty=(path,value,owner,_index=0)=>{var _a,_b;if(path.length===0){return owner;}if(owner.kind===InferenceKind.Any){owner=createShapeInference();}const part=path[_index];if(owner.kind===InferenceKind.Shape){if(_index<path.length-1){let childValue=((_a=owner.properties[part.name])===null||_a===void 0?void 0:_a.value)||createShapeInference();childValue=addInferenceProperty(path,value,childValue,_index+1);owner=addShapeInferenceProperty(part,childValue,owner);}else{const existingInference=((_b=owner.properties[part.name])===null||_b===void 0?void 0:_b.value)||ANY_INFERENCE;if(existingInference.kind===InferenceKind.Shape){value=mergeShapeInference(existingInference,value);}owner=addShapeInferenceProperty(part,value,owner);}}if(owner.kind===InferenceKind.Array){owner=Object.assign(Object.assign({},owner),{value:addInferenceProperty(path,value,owner.value,_index)});}return owner;};const unfurlScopePath=(path,context)=>{let cpath=path;if(!context.scope[path[0].name]){return path;}let entirePath=path;while(true){const property=cpath[0].name;const newCPath=context.scope[property];// if exists, but empty, then the scope is created within the template
if(newCPath){if(newCPath.length===0){return[];}}else{break;}entirePath=[...newCPath,...entirePath.slice(1)];cpath=newCPath;}return entirePath;};const addContextInferenceProperty=(path,value,context)=>Object.assign(Object.assign({},context),{inference:addInferenceProperty(unfurlScopePath(path,context),value,context.inference)});const infer=ast=>{return inferNode(ast,true,{scope:{},inference:createShapeInference()}).inference;};const inferNode=(ast,isRoot,context)=>{switch(ast.nodeKind){case NodeKind.Element:return inferElement(ast,isRoot,context);case NodeKind.Slot:return inferSlot(ast,context);case NodeKind.Fragment:return inferFragment(ast,context);}return context;};const inferElement=(element,isRoot,context)=>{for(const atttribute of element.attributes){context=inferAttribute(atttribute,context);}context=inferChildren(element.children,context);return context;};const inferAttribute=(attribute,context)=>{switch(attribute.attrKind){case AttributeKind.KeyValueAttribute:{if(attribute.value&&attribute.value.attrValueKind===AttributeValueKind.Slot){context=inferJsExpression(attribute.value.script,context);}if(attribute.value&&attribute.value.attrValueKind===AttributeValueKind.DyanmicString){for(const part of attribute.value.values){if(part.partKind===DynamicStringAttributeValuePartKind.Slot){context=inferJsExpression(part,context);}}}break;}case AttributeKind.PropertyBoundAttribute:{context=addContextInferenceProperty([{name:attribute.bindingName,optional:true}],ANY_INFERENCE,context);break;}case AttributeKind.ShorthandAttribute:{context=inferJsExpression(attribute.reference,context);break;}case AttributeKind.SpreadAttribute:{context=inferJsExpression(attribute.script,context,SPREADED_SHAPE_INFERENCE);break;}}return context;};const inferSlot=(slot,context)=>{return inferJsExpression(slot.script,context);};const inferFragment=(fragment,context)=>{return inferChildren(fragment.children,context);};const inferChildren=(children,context)=>children.reduce((context,child)=>inferNode(child,false,context),context);const inferJsExpression=(expression,context,defaultInference=ANY_INFERENCE)=>{switch(expression.jsKind){case JsExpressionKind.Reference:{context=addContextInferenceProperty(expression.path,defaultInference,context);break;}case JsExpressionKind.Node:{context=inferNode(expression,false,context);break;}case JsExpressionKind.Object:{for(const property of expression.properties){context=inferJsExpression(property.value,context,defaultInference);}break;}case JsExpressionKind.Array:{for(const value of expression.values){context=inferJsExpression(value,context,defaultInference);}break;}case JsExpressionKind.Conjunction:{context=inferJsExpression(expression.left,context,defaultInference);context=inferJsExpression(expression.right,context,defaultInference);break;}case JsExpressionKind.Not:{context=inferJsExpression(expression.expression,context,defaultInference);break;}}return context;};
// EXTERNAL MODULE: external "path"
var external_path_ = __webpack_require__(71017);
// EXTERNAL MODULE: ../paperclip-utils/lib/core/url.js
var url = __webpack_require__(16470);
;// CONCATENATED MODULE: ../paperclip/esm/core/utils.js
...
const findResourcesFromConfig=getResources=>fs=>(fromUri,relative)=>{// symlinks may fudge module resolution, so we need to find the real path
const fromPath=fs.realpathSync(new URL(fromUri));const fromPathDirname=external_path_.dirname(fromPath);const configUrl=(0,paperclip_utils.findPCConfigUrl)(fs)(fromUri);// If there's no config, then don't bother looking for PC files. Otherwise we're likely
// to need esoteric logic for resolving PC that I don't think should be supported -- there should
// just be aproach.
if(!configUrl){return[];}const configUri=new URL(configUrl);const config=JSON.parse(fs.readFileSync(configUri,"utf8"));return getResources(fs)(config,external_path_.dirname((0,url/* fileURLToPath */.XN)(configUri))).filter(pathname=>pathname!==fromPath).map(pathname=>{if(relative){const modulePath=getModulePath(configUrl,config,pathname,fromPathDirname);if(!external_path_.isAbsolute(modulePath)){return modulePath;}let relativePath=external_path_.relative(fromPathDirname,modulePath);if(relativePath.charAt(0)!=="."){relativePath="./"+relativePath;}return relativePath;}return (0,url/* pathToFileURL */.Hz)(pathname).href;}).map(filePath=>{return filePath.replace(/\\/g,"/");});};const resolveModuleRoots=fs=>(fromDir,roots=[])=>{const stat=fs.lstatSync(fromDir);const realpath=stat.isSymbolicLink()?fs.realpathSync(fromDir):fromDir;const newStat=realpath===fromDir?stat:fs.lstatSync(realpath);if(!newStat.isDirectory()){return roots;}if(fs.existsSync(external_path_.join(fromDir,"package.json"))){roots.push(fromDir);}else{for(const dirname of fs.readdirSync(realpath)){resolveModuleRoots(fs)(external_path_.join(fromDir,dirname),roots);}}return roots;};const filterAllFiles=fs=>filter=>{const scan=(currentPath,results=[])=>{const stat=fs.lstatSync(currentPath);const realpath=stat.isSymbolicLink()?fs.realpathSync(currentPath):currentPath;const newStat=realpath===currentPath?stat:fs.lstatSync(realpath);if(newStat.isDirectory()){for(const dirname of fs.readdirSync(realpath)){const dirpath=external_path_.join(currentPath,dirname);scan(dirpath,results);}}else{if(filter(currentPath)){results.push(currentPath);}}return results;};return scan;};const resolveResources=fs=>(config,cwd,filterFiles)=>{const sourceDir=config.srcDir==="."?cwd:external_path_.join(cwd,config.srcDir);const filePaths=filterFiles(sourceDir);if(config.moduleDirs){for(const modulesDirname of config.moduleDirs){const moduleDirPath=external_path_.join(cwd,modulesDirname);const moduleRoots=resolveModuleRoots(fs)(moduleDirPath);for(const moduleDir of moduleRoots){// need to scan until there's a package. This covers @organization namespaces.
if(!moduleDir){continue;}const pcConfigPath=external_path_.join(moduleDir,paperclip_utils.PC_CONFIG_FILE_NAME);if(!fs.existsSync(pcConfigPath)){continue;}const moduleConfig=JSON.parse(fs.readFileSync(pcConfigPath,"utf8"));const moduleSources=filterFiles(external_path_.join(moduleDir,moduleConfig.srcDir));filePaths.push(...moduleSources);}}}return filePaths;};const resolveAllPaperclipFiles=findResourcesFromConfig(fs=>(config,cwd)=>{return resolveResources(fs)(config,cwd,filterAllFiles(fs)(filePath=>external_path_.extname(filePath)===".pc"||external_path_.extname(filePath)===".css"));});const resolveAllAssetFiles=findResourcesFromConfig(fs=>(config,cwd)=>{// const ext = `+(jpg|jpeg|png|gif|svg)`;
const exts=[".jpg",".jpeg",".png",".gif",".svg",".ttf"];// const sourceDir = config.srcDir;
return resolveResources(fs)(config,cwd,filterAllFiles(fs)(filePath=>exts.includes(external_path_.extname(filePath))));// if (sourceDir === ".") {
...
});const getModulePath=(configUri,config,fullPath,fromDir)=>{var _a;const configDir=external_path_.dirname((0,url/* fileURLToPath */.XN)(configUri));const moduleDirectory=external_path_.join(configDir,config.srcDir)+"/";if(fullPath.indexOf(moduleDirectory)===0){const modulePath=fullPath.replace(moduleDirectory,"");const relativePath=fromDir&&external_path_.relative(fromDir,fullPath);if(((_a=relativePath===null||relativePath===void 0?void 0:relativePath.match(/\.\.\//g))===null||_a===void 0?void 0:_a.length)||0>0){return modulePath;}}if(config.moduleDirs){for(const moduleDirectory of config.moduleDirs){const fullModulePath=external_path_.join(configDir,moduleDirectory);if(fullPath.indexOf(fullModulePath)===0){return fullPath.replace(fullModulePath,"").substr(1);}}}return fullPath;};// eslint-disable-next-line
const noop=()=>{};
;// CONCATENATED MODULE: ../paperclip/esm/core/delegate.js
// 🙈
const isErrorResult=data=>{return data.error!=null;};var EngineMode;(function(EngineMode){EngineMode[EngineMode["SingleFrame"]=0]="SingleFrame";EngineMode[EngineMode["MultiFrame"]=1]="MultiFrame";})(EngineMode||(EngineMode={}));const mapResult=result=>{if(!result){return result;}if(result.Ok){return result.Ok;}else{return{error:result.Err};}};var EngineDelegateEventType;(function(EngineDelegateEventType){EngineDelegateEventType["Loaded"]="Loaded";EngineDelegateEventType["ChangedSheets"]="ChangedSheets";})(EngineDelegateEventType||(EngineDelegateEventType={}));/*
...
*/class EngineDelegate{constructor(_native,_io,_onCrash=noop){this._native=_native;this._io=_io;this._onCrash=_onCrash;this._listeners=[];this._rendered={};this._documents={};this._graph={};this._onEngineEvent=event=>{if(!this._documents[event.uri]){this._documents[event.uri]=this._io.readFile(event.uri);}if(event.kind===paperclip_utils.EngineDelegateEventKind.Deleted){delete this._rendered[event.uri];}else if(event.kind===paperclip_utils.EngineDelegateEventKind.Evaluated){this._rendered=(0,paperclip_utils.updateAllLoadedData)(this._rendered,event);this._dispatch({kind:paperclip_utils.EngineDelegateEventKind.Loaded,uri:event.uri,data:this._rendered[event.uri]});}else if(event.kind===paperclip_utils.EngineDelegateEventKind.Diffed){const existingData=this._rendered[event.uri];this._rendered=(0,paperclip_utils.updateAllLoadedData)(this._rendered,event);const newData=this._rendered[event.uri];if(existingData.kind===paperclip_utils.EvaluatedDataKind.PC&&newData.kind===paperclip_utils.EvaluatedDataKind.PC){this._handlePCDiff(event,existingData,newData);}}};this._handlePCDiff=(event,existingData,newData)=>{const removedSheetUris=[];const diffData=event.data;for(const{uri}of existingData.importedSheets){if(!newData.allImportedSheetUris.includes(uri)){removedSheetUris.push(uri);}}const addedSheets=[];for(let i=0,{length}=diffData.allImportedSheetUris;i<length;i++){const depUri=diffData.allImportedSheetUris[i];// Note that we only do this if the sheet is already rendered -- engine
...
if(!existingData.allImportedSheetUris.includes(depUri)&&this._rendered[depUri]){addedSheets.push({uri:depUri,index:i,sheet:this._rendered[depUri].sheet});}}if(addedSheets.length||removedSheetUris.length){this._dispatch({uri:event.uri,kind:paperclip_utils.EngineDelegateEventKind.ChangedSheets,data:{// TODO - don't do this - instead include newSheetUris and
// allow renderer to fetch these sheets
newSheets:addedSheets,removedSheetUris:removedSheetUris,allImportedSheetUris:diffData.allImportedSheetUris}});}};this._tryCatch=fn=>{try{return fn();}catch(e){this._onCrash(e);return null;}};this._dispatch=event=>{// try-catch since engine will throw opaque error.
for(const listener of this._listeners){listener(event);}};// only one native listener to for buffer performance
this._native.add_listener(this._dispatch);this.onEvent(this._onEngineEvent);return this;}resolveFile(fromPath,toPath){return this._io.resolveFile(fromPath,toPath);}onEvent(listener){if(listener==null){throw new Error(`listener cannot be undefined`);}this._listeners.push(listener);return()=>{const i=this._listeners.indexOf(listener);if(i!==-1){this._listeners.splice(i,1);}};}parseFile(uri){return mapResult(this._native.parse_file(uri));}lint(uri){return this._native.lint_file(uri);}getVirtualNodeSourceInfo(nodePath,uri){return this._native.get_virtual_node_source_info(nodePath,uri);}getLoadedAst(uri){return this._tryCatch(()=>this._native.get_loaded_ast(uri));}getLoadedDependency(uri){return this._graph[uri]||(this._graph[uri]=this._tryCatch(()=>this._native.get_dependency(uri)));}parseContent(content,uri){return this._tryCatch(()=>mapResult(this._native.parse_content(content,uri)));}purgeUnlinkedFiles(){return this._tryCatch(()=>{const ret=mapResult(this._native.purge_unlinked_files());return ret;});}getVirtualContent(uri){return this._documents[uri];}updateVirtualFileContent(uri,content){if(this._documents[uri]===content){return;}this._graph[uri]=undefined;// only define if successfuly loaded
this._documents[uri]=content;return this._tryCatch(()=>{const ret=mapResult(this._native.update_virtual_file_content(uri,content));return ret;});}inspectNodeStyles(source,screenWidth){return this._native.inspect_node_styles(source.path,source.uri,screenWidth);}getLoadedData(uri){return this._rendered[uri];}getExpressionById(id){return this._native.get_expression_by_id(id);}getAllLoadedData(){return this._rendered;}getLoadedGraph(){const map={};for(const uri in this._rendered){map[uri]=this.getLoadedDependency(uri);}return map;}reset(){this._rendered={};this._documents={};this._native.reset();}open(uri){this._graph[uri]=undefined;// need to load document so that it's accessible via source writer
if(!this._documents[uri]){this._documents[uri]=this._io.readFile(uri);}const result=this._tryCatch(()=>mapResult(this._native.run(uri)));if(result&&result.error){throw result.error;}return this._rendered[uri];}}/**
 * Kept separate from the engine since this is more of a util function for ID inspection
 */const getEngineImports=(uri,delegate)=>{const data=delegate.getLoadedData(uri);if((data===null||data===void 0?void 0:data.kind)===EvaluatedDataKind.PC){const ast=delegate.getLoadedAst(uri);return Object.keys(data.dependencies).reduce((record,id)=>{const depUri=data.dependencies[id];const imp=ast&&(getImportById(id,ast)||getImportBySrc(id,ast));record[id]=Object.assign({uri:imp&&getAttributeStringValue("src",imp),injectStyles:imp&&hasAttribute(INJECT_STYLES_TAG_NAME,imp)},delegate.getLoadedData(depUri));return record;},{});}else{return{};}};
;// CONCATENATED MODULE: ../paperclip/esm/core/index.js

;// CONCATENATED MODULE: ../paperclip/browser.js
const loadEngineDelegate=async(options,onCrash)=>{// need this here since webpack tree shakes it out
await Promise.resolve().then(()=>_interopRequireWildcard(__webpack_require__(21877)));const{NativeEngine}=await Promise.resolve().then(()=>_interopRequireWildcard(__webpack_require__(10700)));const{readFile,fileExists,resolveFile,getLintConfig}=options.io;return new EngineDelegate(NativeEngine.new(readFile,fileExists,resolveFile,getLintConfig,options.mode||EngineMode.MultiFrame),options.io,onCrash||function(e){console.error(e);});};
;// CONCATENATED MODULE: ../paperclip-repl/src/controllers/worker/paperclip.ts
class PaperclipController{constructor(_channels,_io){this._channels=_channels;this._io=_io;}async init(){this._engine=await loadEngineDelegate({io:this._io},this._onCrash);return this._engine;}_onCrash=()=>{};}
// EXTERNAL MODULE: ../paperclip-common/index.js
var paperclip_common = __webpack_require__(28180);
;// CONCATENATED MODULE: ../paperclip-repl/src/controllers/channels.ts
const getFiles=(0,paperclip_common.remoteChannel)("getFiles");const getMainFile=(0,paperclip_common.remoteChannel)("getMainFile");class REPLChannels{constructor(_connection){this._connection=_connection;this.getFiles=getFiles(this._connection);this.getMainFile=getMainFile(this._connection);}}
;// CONCATENATED MODULE: ../paperclip-repl/src/controllers/worker/io.ts
class ReplEngineIO{constructor(_channels){this._channels=_channels;}async init(){this._files=await this._channels.getFiles.call(null);}readFile=filePath=>{return this._files[filePath];};resolveFile=(fromPath,toPath)=>{return (0,url/* pathToFileURL */.Hz)(external_path_.join((0,url/* fileURLToPath */.XN)(external_path_.dirname(fromPath)),toPath)).href;};fileExists=filePath=>{return this._files[filePath]!=null;};}
// EXTERNAL MODULE: ../paperclip-source-writer/lib/index.js
var lib = __webpack_require__(69835);
// EXTERNAL MODULE: ../../node_modules/lodash/lodash.js
var lodash = __webpack_require__(76635);
// EXTERNAL MODULE: ../../node_modules/mime-types/index.js
var mime_types = __webpack_require__(6856);
;// CONCATENATED MODULE: external "os"
const external_os_namespaceObject = require("os");
;// CONCATENATED MODULE: ../tandem-designer/src/state/index.ts
// 2 MB
const MAX_FILE_SIZE=(/* unused pure expression or super */ null && (2*1000*1000));const EDITABLE_MIME_TYPES=["text/plain","image/svg+xml","text/css"];const ALT_MIME_TYPES=["application/vnd.ms-fontobject",// .eot
"font/ttf","font/woff","font/woff2","application/font-woff","application/font-ttf","application/font-woff2"];const MEDIA_MIME_TYPES=["image/png","image/jpeg","image/gif","image/svg+xml","video/quicktime","video/mp4"];const PREVIEW_MIME_TYPES=[...MEDIA_MIME_TYPES,"text/plain","image/svg+xml"];const ACCEPTED_MIME_TYPES=[...ALT_MIME_TYPES,...MEDIA_MIME_TYPES,...EDITABLE_MIME_TYPES];const DEFAULT_FRAME_BOX={width:1024,height:768,x:0,y:0};let FSItemKind;(function(FSItemKind){FSItemKind["FILE"]="file";FSItemKind["DIRECTORY"]="directory";})(FSItemKind||(FSItemKind={}));let SyncLocationMode;(function(SyncLocationMode){SyncLocationMode[SyncLocationMode["None"]=0]="None";SyncLocationMode[SyncLocationMode["Query"]=1]="Query";SyncLocationMode[SyncLocationMode["Location"]=2]="Location";})(SyncLocationMode||(SyncLocationMode={}));let EnvOptionKind;(function(EnvOptionKind){EnvOptionKind["Public"]="Public";EnvOptionKind["Private"]="Private";EnvOptionKind["Browserstack"]="Browserstack";})(EnvOptionKind||(EnvOptionKind={}));const INITIAL_STATE={actions:[],history:{past:[],future:[]},shared:{documents:{}},designer:{useLiteEditor:false,readonly:false,pcFileDataVersion:0,selectedNodeSources:[],selectedNodeStyleInspections:[],expandedNodePaths:[],expandedFilePaths:[],syncLocationMode:SyncLocationMode.Location|SyncLocationMode.Query,sharable:true,ui:{query:{}},highlightNodePath:null,centeredInitial:false,mountedRendererIds:[],toolsLayerEnabled:true,resourceHost:typeof window!=="undefined"?window.location.protocol+"//"+window.location.host+"/file/":null,availableBrowsers:[],currentEngineEvents:{},allLoadedPCFileData:{},boxes:{},zoomLevel:1,selectedNodePaths:[],canvas:{panning:false,showTools:true,scrollPosition:{x:0,y:0},size:{width:0,height:0},mousePosition:null,transform:{x:0,y:0,z:1}}}};const isExpanded=designer=>{return Boolean(designer.ui.query.expanded);};const getActiveFrameIndex=designer=>{return designer.ui.query.frame&&Number(designer.ui.query.frame);};const IS_WINDOWS=external_os_namespaceObject.platform()==="win32";const resetCanvas=canvas=>({...canvas,scrollPosition:{x:0,y:0},transform:{x:0,y:0,z:1}});const mergeBoxesFromClientRects=(boxes,rects)=>{const newBoxes={};for(const nodePath in rects){const{x,y,width,height}=rects[nodePath];const newBox={x,y,width,height};const existingBox=boxes[nodePath];newBoxes[nodePath]=isEqual(existingBox,newBox)?existingBox:newBox;}return newBoxes;};const boxIntersectsPoint=(box,point)=>{return box.x<=point.x&&box.y<=point.y&&box.x+box.width>=point.x&&box.y+box.height>=point.y;};const findBoxNodeInfo=(0,paperclip_utils.memoize)((point,boxes)=>{let bestIntersetingBox;let bestIntersetingNodePath;for(const nodePath in boxes){const box=boxes[nodePath];if(boxIntersectsPoint(box,point)){if(!bestIntersetingBox||nodePath.length>bestIntersetingNodePath.length){bestIntersetingBox=box;bestIntersetingNodePath=nodePath;}}}if(!bestIntersetingBox){return null;}return{nodePath:bestIntersetingNodePath,box:bestIntersetingBox};});const calcFrameBox=(0,paperclip_utils.memoize)(rects=>{let x=0;let y=0;let width=0;let height=0;for(const nodeId in rects){const rect=rects[nodeId];x=Math.min(x,rect.x);y=Math.min(y,rect.y);width=Math.max(width,rect.width);height=Math.max(height,rect.height);}return{x,y,width,height};});const getFSItem=(absolutePath,current)=>{if(current.absolutePath===absolutePath){return current;}if(current.kind===FSItemKind.DIRECTORY){for(const child of current.children){const found=getFSItem(absolutePath,child);if(found){return found;}}}return null;};const getSelectedFrames=designer=>{return designer.selectedNodePaths.map(nodePath=>{const frameIndex=Number(nodePath);return getFrameFromIndex(frameIndex,designer);}).filter(Boolean);};const getFrameFromIndex=(frameIndex,designer)=>{var _designer$allLoadedPC;if(!designer.allLoadedPCFileData){return null;}const preview=(_designer$allLoadedPC=designer.allLoadedPCFileData[designer.ui.query.canvasFile])===null||_designer$allLoadedPC===void 0?void 0:_designer$allLoadedPC.preview;if(!preview){return null;}const frames=preview.kind==VirtualNodeKind.Fragment?preview.children:[preview];return frames[frameIndex];};const getNodeInfoAtPoint=(point,transform,boxes,expandedFrameIndex)=>{return findBoxNodeInfo(getScaledPoint(point,transform),expandedFrameIndex?getFrameBoxes(boxes,expandedFrameIndex):boxes);};const getScopedBoxes=(0,paperclip_utils.memoize)((boxes,scopedElementPath,root)=>{const hoverableNodePaths=getHoverableNodePaths(scopedElementPath,root);return (0,lodash.pick)(boxes,hoverableNodePaths);});const getHoverableNodePaths=(0,paperclip_utils.memoize)((scopedNodePath,root)=>{const scopedNode=scopedNodePath?(0,paperclip_utils.getNodeByPath)(scopedNodePath,root):root;const ancestors=scopedNodePath?(0,paperclip_utils.getNodeAncestors)(scopedNodePath,root):[];const hoverable=[];const scopes=[scopedNode,...ancestors];for(const scope of scopes){addHoverableChildren(scope,true,hoverable);}return hoverable.map(node=>(0,paperclip_utils.getNodePath)(node,root));});const addHoverableChildren=(node,isScope,hoverable)=>{if(!hoverable.includes(node)){hoverable.push(node);}if((0,paperclip_utils.isInstance)(node)&&!isScope){return;}if((0,paperclip_utils.isNodeParent)(node)){for(const child of node.children){addHoverableChildren(child,false,hoverable);}}};const getFrameBoxes=(0,paperclip_utils.memoize)((boxes,frameIndex)=>{const v=(0,lodash.pickBy)(boxes,(value,key)=>{return key.indexOf(String(frameIndex))===0;});return v;});const getPreviewChildren=frame=>{return frame.kind===VirtualNodeKind.Fragment?frame.children:[frame];};const getPreviewFrameBoxes=preview=>{const currentPreview=preview;const frameBoxes=getPreviewChildren(currentPreview).map(frame=>{var _annotations$frame;const annotations=frame.annotations&&computeVirtJSObject(frame.annotations)||{};const box=annotations.frame||DEFAULT_FRAME_BOX;if(((_annotations$frame=annotations.frame)===null||_annotations$frame===void 0?void 0:_annotations$frame.visible)===false){return null;}return{...DEFAULT_FRAME_BOX,...box};});return frameBoxes;};const getCurrentPreview=designer=>{var _designer$ui$query;const currentPCData=designer.allLoadedPCFileData[(_designer$ui$query=designer.ui.query)===null||_designer$ui$query===void 0?void 0:_designer$ui$query.canvasFile];return currentPCData===null||currentPCData===void 0?void 0:currentPCData.preview;};const getCurrentPreviewFrameBoxes=designer=>{const preview=getCurrentPreview(designer);return preview?getPreviewFrameBoxes(preview).filter(Boolean):[];};const getNewFilePath=(name,previousNameOrExt)=>{const ext=previousNameOrExt?previousNameOrExt.split(".").pop():name.includes(".")?name.split(".").pop():"pc";return cleanupPath(name.replace(/\.\w+$/,"")+"."+ext);};const canUpload=files=>{return Array.from(files).every(file=>{if(file.size>MAX_FILE_SIZE){return false;}return ACCEPTED_MIME_TYPES.includes(file.type||String(mime.lookup(file.name)));});};const getAllFrameBounds=designer=>{return mergeBoxes(getCurrentPreviewFrameBoxes(designer));};const INITIAL_ZOOM_PADDING=50;/**
 * Called when file is data -- few conditions need to be met though:
 * canvas is loaded
 * file data is loaded
 */const maybeCenterCanvas=(designer,force)=>{var _designer$canvas$size,_designer$canvas$size2;if(force||!designer.centeredInitial&&designer.allLoadedPCFileData[designer.ui.query.canvasFile]&&(_designer$canvas$size=designer.canvas.size)!==null&&_designer$canvas$size!==void 0&&_designer$canvas$size.width&&(_designer$canvas$size2=designer.canvas.size)!==null&&_designer$canvas$size2!==void 0&&_designer$canvas$size2.height){designer=produce(designer,newDesigner=>{newDesigner.centeredInitial=true;});let targetBounds;const currentFrameIndex=getActiveFrameIndex(designer);if(currentFrameIndex!=null){const frameBoxes=getPreviewFrameBoxes(designer.allLoadedPCFileData[designer.ui.query.canvasFile].preview);targetBounds=frameBoxes[currentFrameIndex];}designer=centerEditorCanvas(designer,targetBounds);return designer;}return designer;};const updateShared=(state,shared)=>{return{...state,shared:{...state.shared,...shared}};};// https://github.com/crcn/tandem/blob/10.0.0/packages/dashboard/src/state/index.ts#L1304
const centerEditorCanvas=(designer,innerBounds,zoomOrZoomToFit=true)=>{if(!innerBounds){innerBounds=getAllFrameBounds(designer);}// no windows loaded
if(innerBounds.x+innerBounds.y+innerBounds.width+innerBounds.height===0){console.warn(` Cannot center when bounds has no size`);return designer;}const{canvas:{transform,size:{width,height}}}=designer;const centered={x:-innerBounds.x+width/2-innerBounds.width/2,y:-innerBounds.y+height/2-innerBounds.height/2};const scale=typeof zoomOrZoomToFit==="boolean"?Math.min((width-INITIAL_ZOOM_PADDING)/innerBounds.width,(height-INITIAL_ZOOM_PADDING)/innerBounds.height):typeof zoomOrZoomToFit==="number"?zoomOrZoomToFit:transform.z;designer=produce(designer,newDesigner=>{newDesigner.canvas.transform=centerTransformZoom({...centered,z:1},{x:0,y:0,width,height},Math.min(scale,1));});return designer;};const cleanupPath=path=>{// just rel directory - no root defined
return path.replace(/\w+:\/\//,"").replace(/\/+/,"/");};const getActivePCData=designer=>designer.allLoadedPCFileData[designer.ui.query.canvasFile];const getAppActivePCData=state=>getActivePCData(state.designer);const getSelectedNodePaths=state=>state.designer.selectedNodePaths;const getInspectionInfo=state=>state.designer.selectedNodeStyleInspections;const pruneDeletedNodes=designer=>{return produce(designer,newDesigner=>{const pruneAry=ary=>{let pruned=false;for(let i=ary.length;i--;){const nodePath=ary[i];if(!getNodeByPath(nodePath,activePCData===null||activePCData===void 0?void 0:activePCData.preview)){pruned=true;ary.splice(i,1);}}return pruned;};const activePCData=getActivePCData(newDesigner);if(pruneAry(newDesigner.selectedNodePaths)){newDesigner.selectedNodeStyleInspections=[];newDesigner.selectedNodeSources=[];}pruneAry(newDesigner.expandedNodePaths);if(newDesigner.scopedElementPath&&!getNodeByPath(newDesigner.scopedElementPath,activePCData.preview)){newDesigner.scopedElementPath=undefined;}});};const getBranchInfo=state=>{var _state$designer$works;return(_state$designer$works=state.designer.workspace)===null||_state$designer$works===void 0?void 0:_state$designer$works.branchInfo;};const getCommitStatus=state=>state.designer.commitProjectStatus;
;// CONCATENATED MODULE: ../paperclip-repl/src/controllers/worker/designer-channel-handler.ts
class DesignerChannelHandler{constructor(_channels,_replChannels){this._channels=_channels;this._replChannels=_replChannels;this._channels.getAllScreens.listen(this._getAllScreens);this._channels.hello.listen(this._hello);this._channels.openFile.listen(this._openFile);this._channels.loadDirectory.listen(this._loadDirectory);this._channels.editCode.listen(this._editCode);this._channels.editPCSource.listen(this._editPCSource);this._channels.inspectNodeStyle.listen(this._inspectNodeStyles);this._channels.loadVirtualNodeSources.listen(this._loadVirtualNodeSources);this._ready=new Promise(resolve=>this._resolveReady=resolve);}init(engine){this._engine=engine;this._engine.onEvent(event=>{this._channels.events.call((0,paperclip_utils.engineDelegateChanged)(event));});this._resolveReady();}_loadVirtualNodeSources=sources=>{return sources.map(info=>{return{virtualNodePath:info.path,source:this._engine.getVirtualNodeSourceInfo(info.path,info.uri)};});};_inspectNodeStyles=async sources=>{return sources.map(source=>[source,this._engine.inspectNodeStyles(source,0)]);};_editPCSource=async mutations=>{const writer=new lib.PCSourceWriter(this._engine);return writer.apply(mutations);};_loadDirectory=async({path})=>{const files=await this._replChannels.getFiles.call(null);return{name:"/",kind:FSItemKind.DIRECTORY,absolutePath:"/",url:"/",children:Object.keys(files).map(filePath=>{return{name:filePath.split("/").pop(),absolutePath:filePath,kind:FSItemKind.FILE,url:filePath};})};};_editCode=({uri,value})=>{this._engine.updateVirtualFileContent(uri,value);};_openFile=async({uri})=>{await this._ready;if((0,paperclip_utils.isPaperclipFile)(uri)){const data=this._engine.open(uri);return{uri,data,document:this._engine.getVirtualContent(uri)};}};_getAllScreens=async()=>{await this._ready;const files=await this._replChannels.getFiles.call(null);return this._engine.getAllLoadedData();};_hello=async()=>{await this._ready;const canvasFile=await this._replChannels.getMainFile.call(null);return{canvasFile,showFullEditor:true,localResourceRoots:["/"],branchInfo:null};};}
;// CONCATENATED MODULE: ../paperclip-repl/src/controllers/worker/index.ts
class REPLWorker{constructor(_connection){this._connection=_connection;}async init(){this._designerChannels=new Channels(this._connection);this._replChannels=new REPLChannels(this._connection);this._channelHandler=new DesignerChannelHandler(this._designerChannels,this._replChannels);this._io=new ReplEngineIO(this._replChannels);await this._io.init();this._paperclip=new PaperclipController(this._designerChannels,this._io);const engine=await this._paperclip.init();this._channelHandler.init(engine);}}
;// CONCATENATED MODULE: ../paperclip-repl/src/controllers/worker/entry.ts
