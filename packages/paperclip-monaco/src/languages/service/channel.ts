  ...
  return (worker: Worker | Window): Channel<TRequest, TResponse> => {
    ...
    const responder = (request: (payload: any) => Promise<any>) => {
      spy(worker, "onmessage", async event => {
        if (event.data.name === name) {
          (worker as any).postMessage({ name, payload: await request(event.data.payload) });
        }
      });
    };
...
export const documentColors = channel<{ uri: string }, ColorInfo[]>("documentColors");
export const updateDocument = channel<{ uri: string, value: string }>("updateDocument");