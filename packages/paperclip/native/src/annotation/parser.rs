...
fn parse_annotation_property<'a, 'b>(context: &mut Context<'a, 'b>) -> Result<ast::AnnotationProperty, ParseError> {
  context.tokenizer.eat_whitespace();
  match context.tokenizer.peek(0)? {
    Token::Word(_) | Token::Byte(_) | Token::Cluster(_) => {
      parse_text_annotation(context)
    }
    Token::At => {
      parse_declaration_property(context)
    }
  }
}


fn parse_text_annotation<'a, 'b>(context: &mut Context<'a, 'b>) -> Result<ast::AnnotationProperty, ParseError> {
  Err(ParseError::unexpected_token(0))
}

fn parse_declaration_property<'a, 'b>(context: &mut Context<'a, 'b>) -> Result<ast::AnnotationProperty, ParseError> {
  Err(ParseError::unexpected_token(0))
}