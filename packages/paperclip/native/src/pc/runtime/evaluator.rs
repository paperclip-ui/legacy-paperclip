...
pub fn traverse_js_expr_css<TEach>(current: &js_ast::Expression, each: &mut TEach) -> Result<(), RuntimeError> where TEach: FnMut(&js_ast::Expression) -> Result<(), RuntimeError> {
  each(current)?;
