...
fn diff_rule(old_rule: &virt::Rule, new_rule: &virt::Rule, index: usize, path: &Vec<usize>, context: &mut Context) {
...
fn get_rule_id(rule: &virt::Rule) -> String {
  match rule {
    virt::Rule::Style(style) => {
      style.selector_text.to_string()
    },
    virt::Rule::Media(media) => {
      format!("@media{}", media.condition_text)
    },
    virt::Rule::Keyframes(keyframes) => {
      format!("@keyframes{}", keyframes.name)
    },
    virt::Rule::FontFace(_) => {
      format!("@fontface")
    },
    virt::Rule::Charset(_) => {
      format!("@charset")
    ...
    virt::Rule::Supports(_) => {
      format!("@supports")
    },
    virt::Rule::Namespace(_) => {
      format!("@namespace")
    },
    virt::Rule::Document(_) => {
      format!("@document")
    }
  }
}