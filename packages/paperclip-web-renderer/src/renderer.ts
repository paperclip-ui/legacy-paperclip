...
import { Node as VirtNode, EngineEvent, EngineEventKind } from "paperclip-utils";
...
export class Renderer {
  ...
  constructor(readonly protocol: string, private _domFactory: DOMFactory = document) {
  ...
  handleEngineEvent = (event: EngineEvent) => {
    switch (event.kind) {
      ...
      case EngineEventKind.Diffed: {
        patchNativeNode(this._stage, event.mutations, this._domFactory, this.protocol);
