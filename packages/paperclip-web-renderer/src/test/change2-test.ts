  ...
  [
    ...
    it(title, async () => {
      ...
      for (const [change] of changes) {
        ...
        // const current = renderFrames(engine.open("entry.pc"), { domFactory: mockDOMFactory });

        // expect(baseline.map(frame => frame.innerHTML).join("")).to.eql(
        //   expectationSanityCheck
        // );

        // const html = combineFrameHTML(renderer.getState());
        // expect().to.eql(combineFrameHTML(baseline.getState()));
      }
    });
  });
});
